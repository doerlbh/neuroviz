...
Path.prototype = path.prototype = {
  ...
  arc: function(x2, y2, r, a0, a1, ccw) {
    ...
    var dx = r * Math.cos(a0), dy = r * Math.sin(a0), x06 = x2 + dx, y06 = y2 + dy, cw = 1 ^ ccw, da2 = ccw ? a0 - a1 : a1 - a0;
    if (r < 0)
      throw new Error("negative radius: " + r);
    if (this._x1 === null) {
      this._ += "M" + x06 + "," + y06;
    ...
    if (da2 > tauEpsilon) {
      ...
      this._ += "A" + r + "," + r + ",0," + +(da2 >= pi3) + "," + cw + "," + (this._x1 = x2 + r * Math.cos(a1)) + "," + (this._y1 = y2 + r * Math.sin(a1));
    }
  },
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    while (++x2 < dx - 1) {
      t0 = t1, t1 = values[x2 + 1] >= value;
      cases[t0 | t1 << 1].forEach(stitch);
    }
    cases[t1 << 0].forEach(stitch);
    while (++y2 < dy - 1) {
      x2 = -1;
      t1 = values[y2 * dx + dx] >= value;
      t2 = values[y2 * dx] >= value;
      cases[t1 << 1 | t2 << 2].forEach(stitch);
      while (++x2 < dx - 1) {
        t0 = t1, t1 = values[y2 * dx + dx + x2 + 1] >= value;
...
function density_default() {
  ...
  function density(data) {
    ...
    data.forEach(function(d, i, data2) {
      var xi = (x2(d, i, data2) + o) * pow2k, yi = (y2(d, i, data2) + o) * pow2k, wi = +weight(d, i, data2);
...
function circle_default() {
  ...
  circle2.center = function(_) {
    return arguments.length ? (center2 = typeof _ === "function" ? _ : constant_default6([+_[0], +_[1]]), circle2) : center2;
  };
...
function resample(project, delta2) {
  function resampleLineTo(x06, y06, lambda04, a0, b02, c0, x12, y12, lambda12, a1, b12, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a3 = a0 + a1, b = b02 + b12, c5 = c0 + c1, m2 = sqrt2(a3 * a3 + b * b + c5 * c5), phi2 = asin(c5 /= m2), lambda22 = abs3(abs3(c5) - 1) < epsilon6 || abs3(lambda04 - lambda12) < epsilon6 ? (lambda04 + lambda12) / 2 : atan2(b, a3), p = project(lambda22, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
function azimuthalInvert(angle2) {
  return function(x2, y2) {
    var z = sqrt2(x2 * x2 + y2 * y2), c5 = angle2(z), sc = sin2(c5), cc2 = cos2(c5);
    return [
      atan2(x2 * sc, z * cc2),
      asin(z && y2 * sc / z)
    ];
...
function arc_default() {
  var innerRadius = arcInnerRadius, outerRadius = arcOuterRadius, cornerRadius = constant_default8(0), padRadius = null, startAngle = arcStartAngle, endAngle = arcEndAngle, padAngle = arcPadAngle, context = null;
  function arc() {
...
function line_default2(x2, y2) {
  var defined = constant_default8(true), context = null, curve = linear_default, output = null;
...
function area_default5(x06, y06, y12) {
  var x12 = null, defined = constant_default8(true), context = null, curve = linear_default, output = null;
...
function zoom_default2() {
  ...
  zoom.translateTo = function(selection2, x2, y2, p, event) {
    zoom.transform(selection2, function() {
      ...
      return constrain(identity2.translate(p02[0], p02[1]).scale(t.k).translate(typeof x2 === "function" ? -x2.apply(this, arguments) : -x2, typeof y2 === "function" ? -y2.apply(this, arguments) : -y2), e, translateExtent);
    }, p, event);
  };
